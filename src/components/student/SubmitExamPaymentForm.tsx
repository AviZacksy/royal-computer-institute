...
import { submitExamPaymentAction } from "@/actions/student/payments";
...
export function SubmitExamPaymentForm({ examId, examTitle, feeAmount }: Props) {
  const [state, action, pending] = useActionState(submitExamPaymentAction, null as ActionState);
    ...
    return (
      <Card className="p-6">
        <div className="text-center space-y-3">
          ...
          <p className="font-extrabold text-[var(--ui-primary)]">Registration Submitted!</p>
  ...
  return (
    <Card className="p-5">
      <div className="mb-6 flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-6 text-center">
        ...
        <p className="font-bold text-[var(--ui-primary)]">Scan to Pay Exam Fee</p>
        <p className="text-sm text-[var(--ui-muted)] mt-1">UPI ID: royalcomputer@upi</p>
      </div>

      <p className="mb-1 font-extrabold text-[var(--ui-primary)]">Register for: {examTitle}</p>
      <p className="mb-4 text-sm text-[var(--ui-muted)]">
        Exam Fee: <span className="font-semibold text-[var(--ui-text)]">₹{feeAmount.toLocaleString("en-IN")}</span>
      </p>
      ...
      <form action={action} className="grid gap-4">
        ...
        <Field label="Amount Paid (₹)" htmlFor="sp-amount">
          <Input
            ...
            placeholder={`e.g. ₹${feeAmount.toLocaleString("en-IN")}`}
          />
        </Field>
        ...
        <Field label="Payment Screenshot (Required)" htmlFor="sp-screenshot">
          <input
            ref={fileRef}
            id="sp-screenshot"
            name="screenshot"
        ...
        <Button type="submit" disabled={pending || feeAmount <= 0}>
          {pending ? "Submitting…" : "Pay & Register"}
        </Button>
      </form>
    </Card>
  );
